...
export class FreightRouteLocationsController {
  constructor(private readonly routeLocationsService: FreightRouteLocationsService) {}
  ...
  async create(@Body() createLocationDto: CreateRouteLocationDto): Promise<FreightRouteLocations> {
  ...
  @ApiResponse({ status: 200, description: 'Return all locations for the route.' })
  async findByRouteId(@Param('routeId') routeId: string): Promise<FreightRouteLocations[]> {
  ...
  @ApiResponse({ status: 200, description: 'Return the latest location for the route.' })
  async getLatestLocation(@Param('routeId') routeId: string): Promise<FreightRouteLocations> {
